...
else if (preg_match('/^[a-zA-Z]+$/x', $_POST["name"]))
...
else if (preg_match('/^[一-龠]+$/A', $_POST["name"]))
...
<?php
//sオプション
if (preg_match('/^b.g.+b.g$/s', $_POST["long"]))
echo '「'.$_POST["long"].'」はbで始まりgで終わる三文字の単語が複数';// .　は任意の１文字
else
    echo '下に文字列を入力'
?>
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST">
    <textarea name="long" cols="50" rows="10"></textarea>
    <input type="submit" value="送信">
</form>
